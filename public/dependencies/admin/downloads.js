...
$(function () {
    ...
    $('#initBtTable').on('check.bs.table uncheck.bs.table check-all.bs.table uncheck-all.bs.table', function () {
        $('#archiveData, #inactiveData, #activeData').prop('disabled', !$('#initBtTable').bootstrapTable('getSelections').length)
        selectedIds = getIdSelections('#initBtTable');
    });

    $('#archiveData').click(function () {
        handleRequest(`${adminUrl}my-iihm/archive-download-data`, selectedIds, 'PUT', {}, '#initBtTable', 'are you sure you want to archive selected Dowload data?');
    });
    
    $('#activeData').click(function () {
        handleRequest(`${adminUrl}my-iihm/active-download-data`, selectedIds, 'PUT', {}, '#initBtTable', 'are you sure you want to active selected Download data?');
    });
    
    $('#inactiveData').click(function () {
        handleRequest(`${adminUrl}my-iihm/inactive-download-data`, selectedIds, 'PUT', {}, '#initBtTable', 'are you sure you want to inactive selected Download data?');
    });

    handleSubmit('#downloadForm');
});
